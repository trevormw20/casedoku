...
export function buildCaseGrid({ seed, culprit, labeledBoxes, sceneBox, decoys = [], dims = DIMS9, targetBlanks = 50, maxTries = 600 }) {
  const { N } = dims;
  for (let t = 0; t < maxTries; t++) {
    ...
    return { solution, givens: dig.givens, labeledCells, sceneCell, decoyCells, removed: dig.removed, tries: t + 1 };
  }
  throw new Error(`buildCaseGrid: no valid grid found in ${maxTries} tries (seed=${seed}, culprit=${culprit})`);
}
